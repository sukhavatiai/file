...
"""Render the video based on specific camera poses. Render a series of images from specific views and then generate a smoothing video. Args: scene: The scene to load. Can be the scene's name or a full path to the training data. width: Resolution width of video height: Resolution width of video n_seconds: Number of seconds fps: FPS of the video render_name: Name of video. Returns: None. A video will be saved at scene root path """
def render_video(resolution, numframes, scene, name, spp, fps, exposure=0):
	...
	for i in tqdm(list(range(min(numframes,numframes+1))), unit="frames", desc=f"Rendering"):
		...
		common.write_image(f"temp/{
      i:04d}.jpg", np.clip(frame * 2**exposure, 0.0, 1.0), quality=100)
